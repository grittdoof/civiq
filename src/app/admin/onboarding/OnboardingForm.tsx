...
interface Props {
  ...
  initialCommunes: CommuneOption[];
  userEmail: string | null;
}

export default function OnboardingForm({ initialPending, initialCommunes, userEmail }: Props) {
  const router = useRouter();
  const [pending, setPending] = useState<PendingRequest | null>(initialPending);
  const [communes] = useState<CommuneOption[]>(initialCommunes);
      ...
      <div style={{ maxWidth: 720, margin: "40px auto", padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--border)", marginBottom: 20 }}>
          <button type="button" onClick={() => setTab("join")} style={tabStyle(tab === "join")}>
            <Search size={14} /> Rejoindre une commune
          </button>
            ...
            <div style={{ marginTop: 12, maxHeight: 280, overflowY: "auto", display: "grid", gap: 6 }}>
              ...
              {communes.length === 0 && (
                <p style={{ padding: 16, textAlign: "center", fontSize: 13, color: "var(--fg-muted)" }}>
                  Aucune commune disponible. Utilisez l&apos;onglet « Créer ma commune ».
                </p>
              )}
            </div>
...
const onbCss = `
  .onb-page { min-height: 100vh; padding: 16px; background: var(--bg); }
`;
