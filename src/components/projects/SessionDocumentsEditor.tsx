...
const TYPES: SessionDocument["type"][] = ["presentation", "rapport", "annexe", "ordre_du_jour", "autre"];

export default function SessionDocumentsEditor({
  commissionId, sessionId, initial, canEdit,
}: Props) {
  const router = useRouter();
  const [docs, setDocs] = useState(initial);
  const [uploading, setUploading] = useState(false);
  const [type, setType] = useState<SessionDocument["type"]>("annexe");
  const [error, setError] = useState<string | null>(null);

  const url = `/api/commissions/${commissionId}/sessions/${sessionId}/documents`;
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const data = (await res.json()) as { document?: SessionDocument; error?: string };
  ...
  async function remove(did: string) {
    ...
    const res = await fetch(`${url}/${did}`, { method: "DELETE" });
    if (res.ok) {
  ...
  return (
    <>
      {docs.length === 0 ? (
        <p className="pj-section-empty">Aucun document joint à cette séance.</p>
      ) : (
      ...
      {canEdit && (
        <div className="pj-doc-upload">
          <select
            value={type}
            onChange={(e) => setType(e.target.value as SessionDocument["type"])}
            className="pj-input"
            disabled={uploading}
          >
            {TYPES.map((t) => (
      ...
      {error && (
        <div className="pj-modal-error" style={{ marginTop: 8 }}>{error}</div>
      )}
    </>
  );
}
